                            ...
                            <div class="timeline timeline-one-side" data-timeline-content="axis" data-timeline-axis-style="dashed">
                                <div class="timeline-block">
                                    ...
                                    <div class="timeline-content">
                                        ...
                                        @can('edit bill')
                                            <div class="Action">
                                                <a href="{{ route('bill.edit',\Crypt::encrypt($bill->id)) }}" class="mx-3 btn btn-sm align-items-center float-right" data-toggle="tooltip" data-original-title="{{__('Edit')}}"><i class="ti ti-edit text-white"></i></a>
                                            </div>
                                        @endcan
                                        <small><i class="ti ti-clock mr-1"></i>{{__('Created on ')}} {{\Auth::user()->dateFormat($bill->bill_date)}}</small>
                                    </div>
                                </div>
                                <div class="timeline-block">
                                    ...
                                    <div class="timeline-content">
                                        ...
                                        @endif
                                        @if($bill->status!=0)
                                            <small><i class="ti ti-clock mr-1"></i>{{__('Sent on')}} {{\Auth::user()->dateFormat($bill->send_date)}}</small>
                                        @else
                                            @can('send bill')
                                    ...
                                    <div class="timeline-content">
                                        ...
                                        @if($bill->status!=0)
                                            @can('create payment bill')
                                                <a href="#" data-url="{{ route('bill.payment',$bill->id) }}" data-ajax-popup="true" data-title="{{__('Add Payment')}}" class="mx-3 btn btn-sm align-items-center float-right" data-toggle="tooltip" data-original-title="{{__('Add Payment')}}"><i class="ti ti-money-bill-alt"></i></a> <br>
                                            @endcan
                                        @endif
        ...
        @if($bill->status!=0)
            <div class="row justify-content-between align-items-center mb-3">
                <div class="col-10 offset-1 d-flex align-items-center justify-content-between justify-content-md-end">
                    ...
                    @endif
                    <div class="all-button-box mx-2">
                        <a href="{{ route('bill.resent',$bill->id) }}" class="btn btn-success">
                            {{__('Resend Bill')}}
                        </a>
                    </div>
                    <div class="all-button-box">
                        <a href="{{ route('bill.pdf', Crypt::encrypt($bill->id))}}" target="_blank" class="btn btn-success">
                            {{__('Download')}}
                        </a>
                    </div>
                    {{-- @if($bill->getDue() > 0)
                                ...
                                <tr>
                                    ...
                                    <td class="text-dark">
                                        @can('delete bill product')
                                            <a href="#" class="mx-3 btn btn-sm align-items-center" data-toggle="tooltip" data-original-title="{{__('Delete')}}" data-confirm="{{__('Are You Sure?').'|'.__('This action can not be undone. Do you want to continue?')}}" data-confirm-yes="document.getElementById('delete-form-{{$payment->id}}').submit();">
                        ...
                        <table class="table ">
                            ...
                            @forelse($bill->debitNote as $key =>$debitNote)
                                <tr>
                                    ...
                                    <td>
                                        @can('edit debit note')
                                            <a data-url="{{ route('bill.edit.debit.note',[$debitNote->bill,$debitNote->id]) }}" data-ajax-popup="true" data-title="{{__('Add Debit Note')}}" href="#" class="mx-3 btn btn-sm align-items-center" data-toggle="tooltip" data-original-title="{{__('Edit')}}">
                                                <i class="ti ti-edit text-white"></i>
                                            </a>
                                        @endcan
                                        @can('delete debit note')
                                            <a href="#" class="mx-3 btn btn-sm align-items-center " data-toggle="tooltip" data-original-title="{{__('Delete')}}" data-confirm="{{__('Are You Sure?').'|'.__('This action can not be undone. Do you want to continue?')}}" data-confirm-yes="document.getElementById('delete-form-{{$debitNote->id}}').submit();">
                                                <i class="ti ti-trash text-white"></i>
                                            </a>
